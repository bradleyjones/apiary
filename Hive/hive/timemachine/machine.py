...
class Machine(Process):
  ...
  def run(self):
      #### Control Rig Setup #### 
      credentials = pika.PlainCredentials(self.config['Rabbit']['username'], self.config['Rabbit']['password'])
...
class Writer(threading.Thread):
    ...
    def run(self):
        ...
        resp = rpcSender.send_request('THINGS', 'honeycomb', '{}', 'SOMETHING', 'timemachineworker', key='honeycomb')
        for log in resp.data:
          self.channel.basic_publish(exchange='', routing_key=self.queue, body=log)
     
        # Start listening for live data to forward
        result = channel.queue_declare(exclusive=true)
        self.buff = result.method.queue
        channel.queue_bind(exchange='apiary', queue=self.buff, routing_key='agent.*.data')
